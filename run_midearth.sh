...
def solve(dataset_path, direction="forward"):
    ...
    if pid == 0:
        if direction == "forward":
            os.execvp("/usr/games/quiz", ["quiz", "Middle-Earth", "cap"])
        else:
            os.execvp("/usr/games/quiz", ["quiz", "cap", "Middle-Earth"])
...
solve("/usr/share/games/bsdgames/quiz/midearth", "forward")
solve("/usr/share/games/bsdgames/quiz/midearth", "reverse")
PY_EOF
