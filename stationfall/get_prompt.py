...
def w():
    ...
    while True:
        try:
            idx = child.expect(['>', r'\*\*\*MORE\*\*\*', r'\[Hit.*ENTER\.\]', r'\[Hit RETURN or ENTER\.\]'])
            out += child.before
            if idx >= 1:
                child.sendline('')
            else:
...
cmds_end = [
    ...
    "KICK GRATE", "TURN JAMMER ON", "TURN JAMMER OFF", "UP",
]
for c in cmds_end: cmd(c)

child.sendline("SHOOT FLOYD")
try:
    child.expect('SOME_STRING_THAT_NEVER_MATCHES', timeout=2)
except pexpect.TIMEOUT:
    print("BUFFER AFTER SHOOT FLOYD:")
