...
m = check_moves()
print(f"Current move: {m}")
while m < 3290:
    run_cmd("WAIT")
    m = check_moves()
    
# Now plant the bomb in the Commander's Quarters
print(f"Planting bomb at move {m}")
cmds_bomb = [
    "EAST", "OPEN THERMOS", "GET EXPLOSIVE",
    "CONNECT TIMER TO DETONATOR", "CONNECT DETONATOR TO EXPLOSIVE", "PUT EXPLOSIVE IN HOLE",
    "DROP TIMER", "DROP DETONATOR", "DROP THERMOS", "TURN HEADLAMP ON"
]
for cmd in cmds_bomb:
    print(run_cmd(cmd))

# Now wait right up until Plato is about to attack
m = check_moves()
while m < 3314:
    run_cmd("WAIT")
    m = check_moves()

print(f"Setting timer at move {m}")
# We set timer to 5, which means it blows up 5 moves from now (3319)
...
if plato_attacked:
    ...
    for _ in range(5):
        ...
        print(out)
        if "exploded" in out or "blown apart" in out or "muscular control has returned" in out:
            print("Plato dead!")
            break
            
    # Wait for the bomb blast in the room next door
    for _ in range(20):
        out = run_cmd("WAIT")
        print(out)
        if "muscular control has returned" in out or "deafening" in out or "violently rocks" in out:
            print("Bomb exploded and we recovered!")
            break
