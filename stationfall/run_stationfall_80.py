...
def w():
    ...
    while True:
        try:
            # We match the final ***MORE*** and empty prompts
            idx = child.expect(['>', r'\*\*\*MORE\*\*\*', r'\[Hit.*ENTER\.\]', r'\[Hit RETURN or ENTER\.\]', pexpect.EOF], timeout=5)
            if idx == 4:
                return out + child.before
            out += child.before
            if idx >= 1:
                child.sendline('')
            else:
...
cmds_end = [
    ...
    "KICK GRATE", "TURN JAMMER ON", "TURN JAMMER OFF", "UP", "SHOOT FLOYD"
]
...
print("FINAL OUTPUT:")
out_final = cmd("PUT FOIL ON PYRAMID")
print(out_final)
print("SCORE OUTPUT:")
out_score = cmd("SCORE")
print(out_score)
