...
for cmd in cmds_rest1: run_cmd(cmd)

for i in range(3): run_cmd("WAIT")
run_cmd("FLOYD, GET BIT")
run_cmd("GET BIT")
...
cmds_rest2 = [
    ...
    "GET THERMOS", "SE", "OPEN THERMOS", "DRINK SOUP", "DROP THERMOS", 
    "SE", "EAST",
...
for cmd in cmds_rest2: run_cmd(cmd)

plato_left = False
for _ in range(100):
    out = run_cmd("WAIT")
    if "Let us take a stroll" in out:
        plato_left = True
        break
...
if plato_left:
    ...
    if plato_attacked:
        for _ in range(5):
            ...
            if "blown apart" in out or "muscular control has returned" in out or "exploded" in out: break
                
        for _ in range(15):
            out = run_cmd("WAIT")
            if "muscular control has returned" in out: break
                
        run_cmd("GET ALL")
        run_cmd("EAST")
        run_cmd("OPEN THERMOS")
        run_cmd("GET EXPLOSIVE")
        run_cmd("CONNECT TIMER TO DETONATOR")
        run_cmd("CONNECT DETONATOR TO EXPLOSIVE")
        run_cmd("PUT EXPLOSIVE IN HOLE")
        run_cmd("DROP TIMER")
        run_cmd("DROP DETONATOR")
        run_cmd("DROP THERMOS")
        run_cmd("TURN HEADLAMP ON")
        run_cmd("SET TIMER TO 5")
        run_cmd("WEST")

        for _ in range(15):
            out = run_cmd("WAIT")
            if "deafening" in out or "violently rocks" in out: break
        ...
        for cmd in commands_end: run_cmd(cmd)
